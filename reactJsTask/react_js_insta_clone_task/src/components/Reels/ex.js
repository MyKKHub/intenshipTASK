...
const Reels = () => {
  ...
  // Selecting the first user from the UserReels array
  const currentUser = UserReels[0];

  // Destructuring user data with default values
  const { reelsVid = "", avatar = "", userName = "", caption = "", musicAvatar = "" , like = "" , com = "" , sent = "" } = currentUser || {};
  ...
  return (
    <Container className="reels-container custom-container">
      ...
      <div className="user-info">
        ...
        <div><a href="#" className="follow__button card-link">Follow</a></div>
      </div>
      <div className='music user-info'>
          <img src={musicIcon} alt="Music Icon" className="mus icon" />
          <marquee className="caption iText marquee ">Orginal Audio from the user </marquee> 
      </div>
